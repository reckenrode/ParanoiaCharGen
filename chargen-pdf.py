...
def get_spy_info(society):
	...
	if society.cover != None:
		results.append(Paragraph('You are a member of the Illuminati.', normal, 'a'))
		results += get_spy_info(society.cover)
	elif society.spyon != None:
		results.append(Paragraph('You are spying on %s for %s.' % (society.spyon.name, society.name), normal, 'a'))
...
try:
	...
	if char.group.cover != None:
		notesBlock.append(Paragraph('You are a spy for Internal Security [%s].' % escape(`char.group.firm`), normal, 'a'))
	elif char.group.spyon != None:
		notesBlock.append(Paragraph('You are an industrial spy or saboteur, and your target is %s.' % escape(`char.group.spyon`), normal, 'a'))
